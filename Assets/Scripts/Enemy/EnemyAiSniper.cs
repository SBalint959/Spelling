...
public class EnemyAiSniper : MonoBehaviour
{
    ...
    void PerformAttack()
    {
        ...
        Vector3 spawnPosition = transform.position + directionToPlayer * 1.2f + Vector3.up * 1.5f + Vector3.left * 0.5f;

        Rigidbody rb = Instantiate(projectile, spawnPosition, spawnRotation).GetComponent<Rigidbody>();
        Vector3 shootForce = directionToPlayer * 60f + Vector3.up * 3.5f + Vector3.right *1.5f;
